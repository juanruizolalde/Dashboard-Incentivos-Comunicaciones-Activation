#!/usr/bin/env python3
"""
Actualiza index.html (dashboard Activation) leyendo los datos directamente
desde el Google Sheet alimentado por Aleph.

Reemplaza exactamente el mismo bloque `const DATA = {...};` que antes se
regeneraba a mano procesando los CSVs exportados de Snowflake -- la logica
de agregacion es identica a la del script process.py original.

Requiere la variable de entorno GCP_SA_KEY con el JSON completo de una
service account de Google con acceso de Lector al sheet. Ver SETUP.md.
"""
import os
import re
import sys
...
# Configuracion
# ---------------------------------------------------------------------------
SHEET_ID = "12ESyFI5vrAzMRJ03FIzHnV8skkK4w4AQvZFcZ9MItVk"
TAB_Q1 = "Activation query 1"
TAB_Q2 = "Activation query 2"
...
def main():
    ...
    date_range = {"start": min(weeks), "end": max(weeks)}
    ...
    DATA = {
        ...
        "incentive_classification": "Activation",
        # se muestra tal cual en el header del dashboard ("Actualizado: ...")
